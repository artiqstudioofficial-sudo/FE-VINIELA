...
const CareersDetailPage: React.FC = () => {
    ...
    if (isSubmitted) {
        return (
            <div className="container mx-auto px-6 py-20 min-h-[60vh] flex flex-col justify-center items-center text-center animate-fade-in-up">
                <i className="fa-solid fa-check-circle text-green-500 fa-4x w-16 h-16 mb-6" aria-hidden="true"></i>
                <h1 className="text-4xl font-bold text-viniela-dark">{t.careersDetail.applicationSuccessTitle}</h1>
                <p className="mt-4 max-w-lg text-lg text-viniela-gray">{t.careersDetail.applicationSuccessMessage}</p>
                <Link to="/careers" className="mt-8 px-6 py-3 bg-viniela-gold text-white font-semibold rounded-lg shadow-md hover:bg-viniela-gold-dark transition-all">
    ...
    if (!job) {
        return <div className="container mx-auto px-6 py-20 text-center min-h-[50vh]">{t.careersDetail.notFound}</div>;
    }

    return (
        <div className="bg-white animate-fade-in-up">
            <div className="container mx-auto px-6 py-12 max-w-4xl">
                <Link to="/careers" className="inline-flex items-center text-viniela-gray hover:text-viniela-gold transition-colors mb-8 group font-semibold">
                     <i className="fa-solid fa-chevron-left mr-2 transform group-hover:-translate-x-1 transition-transform" aria-hidden="true"></i>
                    {t.careersDetail.backToCareers}
                </Link>

                <header className="mb-10 text-center border-b pb-8">
                    <h1 className="text-4xl md:text-5xl font-extrabold text-viniela-dark leading-tight">{job.title[language]}</h1>
                    <div className="flex flex-wrap justify-center items-center gap-x-6 gap-y-2 text-viniela-gray mt-4">
                        <span className="flex items-center text-lg"><i className="fa-solid fa-briefcase w-5 h-5 mr-2" aria-hidden="true"></i>{t.admin.jobTypes[job.type]}</span>
                        <span className="flex items-center text-lg"><i className="fa-solid fa-location-dot w-5 h-5 mr-2" aria-hidden="true"></i>{job.location[language]}</span>
                    </div>
                </header>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                     <article className="lg:col-span-2 space-y-10">
                        ...
                        <section>
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <h2 className="text-2xl font-bold text-viniela-dark">{t.careersDetail.jobDescription}</h2>
                            </div>
                            <div
                        ...
                        <section>
                             <div className="flex items-center gap-4 mb-4">
                                ...
                                <h2 className="text-2xl font-bold text-viniela-dark">{t.careersDetail.responsibilities}</h2>
                            </div>
                            <div
                                className="prose prose-lg max-w-none text-viniela-gray pl-16"
                        ...
                        <section>
                             <div className="flex items-center gap-4 mb-4">
                                ...
                                <h2 className="text-2xl font-bold text-viniela-dark">{t.careersDetail.qualifications}</h2>
                            </div>
                            <div
                                className="prose prose-lg max-w-none text-viniela-gray pl-16"
                    ...
                    <aside className="lg:col-span-1">
                        <div className="bg-viniela-silver p-6 rounded-xl shadow-md sticky top-28">
                            <h3 className="text-2xl font-bold text-viniela-dark mb-4">{t.careersDetail.applicationForm}</h3>
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div>
                                    <label className="form-label">{t.careersDetail.fullName}</label>
                                    <input type="text" name="name" onChange={handleInputChange} className="form-input" required />
                                </div>
                                <div>
                                    <label className="form-label">{t.careersDetail.email}</label>
                                    <input type="email" name="email" onChange={handleInputChange} className="form-input" required />
                                </div>
                                <div>
                                    <label className="form-label">{t.careersDetail.phone}</label>
                                    <input type="tel" name="phone" onChange={handleInputChange} className="form-input" required />
                                </div>
                                <div>
                                    <label className="form-label">{t.careersDetail.resume}</label>
                                    <input type="file" onChange={handleFileChange} accept=".pdf,.doc,.docx" className="form-input file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-viniela-gold/10 file:text-viniela-gold hover:file:bg-viniela-gold/20" required />
                                ...
                                <div>
                                    ...
                                    <textarea name="coverLetter" rows={4} onChange={handleInputChange} className="form-input" />
                                </div>
                                <button type="submit" className="w-full btn-primary mt-4">{t.careersDetail.submitApplication}</button>
                            </form>
                        </div>
                    </aside>
                </div>
