...
const FaqItem: React.FC<FaqItemProps> = ({ question, answer, isOpen, onClick }) => (
  <div className="border-b border-gray-200 last-of-type:border-b-0">
    ...
    >
      ...
      <i className={`fa-solid fa-chevron-down w-6 h-6 text-viniela-gold flex-shrink-0 transform transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} aria-hidden="true"></i>
    </button>
    <div 
        className={`grid transition-all duration-500 ease-in-out ${isOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}
    >
      <div className="overflow-hidden">
        <div className="pb-5 pr-4 text-viniela-gray prose" dangerouslySetInnerHTML={{ __html: answer }} />
      </div>
    </div>
  </div>
);
            ...
            <main className="pb-20">
                <div className="container mx-auto px-6 max-w-4xl space-y-12">
                    {categories.map((category, catIndex) => (
                        <div key={catIndex}>
                             <h2 className="text-3xl font-bold text-viniela-dark text-center mb-8">{category.categoryTitle}</h2>
